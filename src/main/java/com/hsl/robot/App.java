...
public class App {
        ...
        for (int i = 0, len = AddMsgList.size(); i < len; i++) {
            ...
            if (msgType == 51) {
                ...
                if (SpecialUsers.contains(msg.getString("ToUserName"))) {
                    ...
                    String repeat = saveMsgAndRepeat(content);
                    webwxsendmsg(repeat, msg.getString("FromUserName"));
                    LOGGER.info("自动回复 " + repeat);
                }
            } else if (msgType == 3) {
    ...
    /**
     * TODO 保存聊天记录到数据库并根据内容回复，暂时写入到本地文本并回复相同的文本
     */

    private String saveMsgAndRepeat(String content) {
        saveMsg(content);
     ...
     * TODO 保存聊天记录到txt
     */
    private void saveMsg(String content) {
